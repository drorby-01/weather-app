...
export async function getWather(cityName: string): Promise<IWheater> {
  ...
  try {
    const { data }: any = await Api.apiGetCall(
      `https://dataservice.accuweather.com/currentconditions/v1/${locationKey}?apikey=ocrDdRGpBaGDnjg0FzOSnKAgBU9LGd08`
    );
...
export async function getFiveDayWeather(
  cityKey: string = DEFAULTKEY
): Promise<string[]> {
  // i will get it from redux
  const { data }: any = await Api.apiGetCall(
    `https://dataservice.accuweather.com/forecasts/v1/daily/5day/${cityKey}?apikey=ocrDdRGpBaGDnjg0FzOSnKAgBU9LGd08&metric=true`
  );
